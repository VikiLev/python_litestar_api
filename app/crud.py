...
# GET all
async def get_all_tests(session: AsyncSession) -> list[dict]:
    try:
...
# GET by ID
async def get_test_by_id(test_id: int, session: AsyncSession) -> dict | None:
    try:
...
async def create_test(name: str, session: AsyncSession, description: str | None = None) -> dict:
...
# DELETE
async def delete_test(test_id: int, session: AsyncSession) -> bool:
    test = await session.get(TestAPI, test_id)
    if not test:
        return False
